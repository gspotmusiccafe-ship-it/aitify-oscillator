...
# --- [1] SECRET ADMIN PORTAL (THE CLOSED DOOR) ---
...
def minting_suite():
    ...
    <html>
    <head>
        <title>AITIFY | ADMIN ROOM</title>
        <style>
            :root { --green: #00ff33; }
            body { background: #010101; color: var(--green); font-family: 'IBM Plex Mono'; padding: 40px; }
            .station { border: 1px solid var(--green); padding: 30px; background: #0a0a0a; box-shadow: 0 0 20px rgba(0,255,51,0.5); max-width: 600px; margin: auto; }
    ...
    <body>
        <div class="station">
            <h1 style="text-align:center;">PRIVATE MINTING SUITE</h1>
            <p style="font-size:10px; text-align:center; opacity:0.6;">ONLY OCCUPANT: G. SMOOTH</p>
            <form action="/stock_asset" method="post" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn">MINT & BROADCAST LIVE</button>
            </form>
            <br>
            <a href="/" style="color:var(--green); font-size:10px; text-decoration:none;">EXIT TO PUBLIC FLOOR >></a>
        </div>
    </body>
    </html>
    ''')

# --- [2] THE SYNCED STOCKING ---
...
def index():
    ...
    <head>
        ...
        <style>
            ...
            .radio-bar { background: #0a0a0a; border-bottom: 2px solid var(--green); padding: 25px 50px; display: grid; grid-template-columns: 110px 1fr; gap: 30px; align-items: center; box-shadow: 0 10px 30px rgba(0,0,0,0.5); z-index: 10; position: relative; }
            ...
            .ticker { font-size: 2.8em; color: var(--green); font-weight: 900; letter-spacing: -3px; }
                ...
                if (data.roster) {
                    document.getElementById('floor').innerHTML = data.roster.map((i, idx) => `
                        <div class="asset-row" onclick="broadcast('${i.song}', '${i.audio}', '${i.image}')">
                            ...
                            <button class="trade-btn" onclick="event.stopPropagation(); alert('Trade Logged')">TRADE</button>
                        </div>
                    `).join('');
                }
            }
...
def get_data():
    try:
        ...
        roster = [{"song": r[0].upper(), "audio": r[1], "image": r[2], "principal": "{:.2f}".format(float(r[3])), "current_price": "{:.2f}".format(float(r[3]) * 1.4 + random.uniform(-0.03, 0.03))} for r in cur.fetchall()]
