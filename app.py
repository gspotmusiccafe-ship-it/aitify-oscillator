...
def index():
    ...
    <html>
    <head>
        <title>MUSIC MONEY MARKET | V45</title>
        <style>
            :root { --emerald: #50C878; --red: #ff3300; --bg: #030303; }
            ...
            .ignite-btn { background: var(--emerald); color: #000; border: none; padding: 18px; font-weight: 900; cursor: pointer; text-transform: uppercase; margin-bottom: 20px; box-shadow: 0 0 15px var(--emerald); letter-spacing: 1px; }
            .trading-floor { background: #050505; overflow-y: auto; padding: 40px; }
            .pace-card { background: #080808; border: 1px solid #1a1a1a; padding: 35px; display: flex; flex-direction: column; margin-bottom: 40px; min-height: 680px; box-shadow: 0 30px 60px rgba(0,0,0,0.8); }
            ...
            /* HIGH TECH GRID OSCILLATOR */
            .void-filler-graph { 
                width: 100%; height: 380px; background: #000; border: 1px solid #111; position: relative; margin-bottom: 30px; overflow: hidden;
                background-image: linear-gradient(#111 1px, transparent 1px), linear-gradient(90deg, #111 1px, transparent 1px);
                background-size: 20px 20px;
            }
            canvas { width: 100%; height: 100%; }
            .mbbo-dock { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px; }
            .mbbo-btn { padding: 25px; font-weight: bold; border: 1px solid #333; background: #000; color: #fff; cursor: pointer; text-transform: uppercase; font-size: 11px; border-top: 6px solid #444; }
        ...
        <script>
            ...
            function updatePulse(idx, price) {
                ...
                if (charts[idx].length > 600) charts[idx].shift();
            ...
            function ignite(audio, img, title) {
                ...
                player.src = audio; player.load(); player.play().catch(e => console.log("Signal blocked."));
            }
            function unlock() {
