...
def minting_suite():
    ...
    <head>
        ...
        <style>
            ...
            input, textarea { width: 100%; background: #000; border: 1px solid #333; color: #fff; padding: 10px; margin-top: 10px; }
            ...
            .nav-link { color: var(--bloomberg-green); text-decoration: none; font-size: 10px; margin-bottom: 20px; display: block; }
        </style>
    </head>
    <body>
        ...
        <div class="grid">
            <div class="station">
                <h3>[1] LYRIC & IMAGE ENGINE</h3>
                <input type="text" placeholder="ENTER VIBE / PROMPT">
                <button class="action-btn">GENERATE ASSETS</button>
            </div>
            <div class="station">
                ...
                <form action="/stock_asset" method="post" enctype="multipart/form-data">
                    ...
                    <div style="font-size:10px; margin-top:10px;">AUDIO: <input type="file" name="audio" required></div>
                    <div style="font-size:10px; margin-top:10px;">COVER: <input type="file" name="image" required></div>
...
def stock_asset():
    ...
    a_fn, i_fn = secure_filename(audio.filename), secure_filename(image.filename)
...
def index():
    ...
    <head>
        ...
        <style>
            ...
            .art-box { width: 100px; height: 100px; border: 1px solid var(--bloomberg-green); background: #111; }
            ...
            .price { font-size: 2em; color: var(--bloomberg-green); font-weight: 900; }
            .btn { background: var(--bloomberg-green); color: #000; border: none; padding: 10px; font-weight: bold; cursor: pointer; width: 100%; }
        </style>
        <script>
            async function load() {
                ...
                document.getElementById('floor').innerHTML = data.roster.map(i => `
                    <div class="row" onclick="play('${i.song}', '${i.audio}', '${i.image}')">
                        ...
                        <div><b>${i.song}</b></div>
                        <div style="color:var(--bloomberg-green);">$${i.principal}</div>
                        <div class="price">$${i.current_price}</div>
                        <button class="btn" onclick="alert('Trade Sent')">TRADE</button>
                    </div>
                `).join('');
            }
            function play(s, a, i) {
    ...
    <body>
        <div class="header">
            <img id="art" class="art-box" src="">
            <div>
                <b id="title" style="font-size:1.5em;">SELECT ASSET</b><br>
