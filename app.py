...
# --- [1] PRIVATE STUDIO (FIREBASE BRIDGE) ---
    ...
    <body>
        <div class="station">
            ...
            <form action="/stock_asset" method="post">
                <input type="text" name="title" placeholder="ASSET TITLE (e.g. SUPERFLY)" required>
                <input type="text" name="artist" placeholder="ARTIST NAME (e.g. MS BUTTA)" required>
                <input type="number" step="0.01" name="price" placeholder="MINT PRICE" required>
                <input type="text" name="audio_url" placeholder="PASTE FIREBASE AUDIO URL" required>
                <input type="text" name="image_url" placeholder="PASTE FIREBASE IMAGE URL" required>
                <button type="submit" class="btn">MINT TO THE FLAME</button>
            </form>
        </div>
    </body>
    </html>
...
def stock_asset():
    try:
        ...
        return redirect('/')
    except Exception as e: return f"STUDIO ERROR: {str(e)}"

# --- [3] THE FLAME EXCHANGE (LIVE FEED) ---
@app.route('/')
def index():
    return render_template_string('''
    <!DOCTYPE html>
    ...
    <head>
        ...
        <script>
                ...
                if (data.roster) {
                    document.getElementById('floor').innerHTML = data.roster.map((i, idx) => `
                        <div class="asset-row" onclick="broadcast('${i.song}', '${i.audio}', '${i.image}')">
                            ...
                            <div><b>${i.song}</b><br><span style="color:var(--green); font-size:9px;">ON-AIR</span></div>
            ...
            function broadcast(title, audioSrc, img) {
                ...
                player.src = audioSrc; player.load(); player.play();
            }
    ...
    <body>
        <div class="radio-bar">
            ...
            <div>
                <span style="color:var(--green); font-size:10px; letter-spacing:5px;">97.7 THE FLAME</span><br>
                <b id="now-playing" style="font-size:2em;">SELECT ASSET</b><br>
